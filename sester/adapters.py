...
protokolün çekirdek-akışını *yerel* taklit eder:
  ...
  UCP (Universal Commerce Protocol — Google+Shopify, 2026; protocol-delta register v3):
    ...
    Intent:   {intent_id, buyer (cüzdan-adresi), merchant (satıcı-kimliği),
               line_item: {resource, amount_minor, currency}, valid_to,
               signature?}
    SESTER katkısı: manifest-teşhiri *imzalı-zarfla* bağlanır — satıcı
    issue_ucp_checkout ile üretir; alıcı verify_ucp_checkout ile
    ChargeIntent'e çevirir (aynı nonce/quota/kanıt-çekirdeği).

Her üç adaptör de şema-Registry'sine takılır: başlık-önekinden
...
class ChargeIntent:
    ...
    protocol: str         # "ap2" | "acp" | "pugio0" | "exact-sester" | "exact"
...
def sign_mandate_jws(mandate_body: dict[str, Any], *, alg: str, key: Any,
    ...
    try:
        ...
        raise AdapterError("ES256 için 'cryptography' gerekir: pip install 'sester[jws]'") from e
    try:
...
@dataclass(frozen=True)
class UcpCheckout:
    """UCP web-monetization zarfı — satıcı-mühürlü tek-çekim satın-alma.
    Zarf satıcı anahtarıyla JWS-mühürlü gelir; buyer alanı ajan-kimliğidir
    (non-custodial tez). nonce = intent_id → kalıcı replay-koruması."""
    intent_id: str
    buyer: str
    merchant: str
    line_item: dict[str, Any]
    ...
    @classmethod
    def parse(cls, header: str) -> "UcpCheckout":
        obj = _b64_json(header, "UCP-Checkout")
        for k in ("intent_id", "buyer", "merchant", "line_item"):
            if not obj.get(k):
                raise AdapterError(f"ucp.{k} eksik")
        if not isinstance(obj["line_item"], dict):
            raise AdapterError("ucp.line_item nesne değil")
        return cls(
            intent_id=str(obj["intent_id"]),
            buyer=str(obj["buyer"]).lower(),
            merchant=str(obj["merchant"]),
            line_item=obj["line_item"],
            signature=str(obj.get("signature", "")),
            raw=obj,
        )
    ...
    def authorize(self, resource: str, *, now: float | None = None) -> ChargeIntent:
        """Satın-alma niyeti bu kaynağa ait değilse ret (kazıma-kapanı)."""
        _check_window(self.raw, now)
        li = self.line_item
        if str(li.get("resource", "")).lower() != (resource or "").lower():
            raise AdapterError("UCP line_item kaynağı istenen kaynak değil")
        try:
            amount_minor = int(li["amount_minor"])
        except (KeyError, TypeError, ValueError) as e:
            raise AdapterError("UCP line_item.amount_minor bozuk") from e
        if amount_minor <= 0:
            raise AdapterError("UCP line_item.amount_minor pozitif olmalı")
        return ChargeIntent(
            agent=self.buyer, resource=resource, amount_minor=amount_minor,
            currency=str(li.get("currency", "USDC")), nonce=self.intent_id,
            protocol="ucp", raw=self.raw,
        )

    def verify(self, *, resolve_key: KeyResolver | None = None,
               require_signature: bool = False,
               expected_merchant: str | None = None) -> None:
        """Satıcı-mührü doğrulaması. expected_merchant verilirse zarfın
        merchant alanı ile bağlanır (manifest-teşhir sahteliği kapanır).
        İmzasız zarf yalnız require_signature=False'ta kabul edilir."""
        if expected_merchant is not None and self.merchant != expected_merchant:
            raise AdapterError("UCP merchant beklenen satıcı değil")
        if not self.signature:
            if require_signature:
                raise SignatureRequiredError("UCP checkout imzasız — production reddi")
            return
        if resolve_key is None:
            if require_signature:
                raise SignatureRequiredError("key_resolver yok — UCP imzası doğrulanamadı")
            return
        verify_mandate_jws(
            self.signature, resolve_key=resolve_key,
            expected_body={k: v for k, v in self.raw.items() if k != "signature"},
...
def issue_ucp_checkout(*, intent_id: str, buyer: str, merchant: str,
                       resource: str, amount_minor: int,
                       currency: str = "USDC", ttl_seconds: float = 3600.0,
                       alg: str = "HS256", key: Any = None,
                       kid: str | None = None,
                       valid_from: float | None = None) -> str:
    """Satıcı-tarafı üretici: imzalı UCP-Checkout başlığı döndürür
    (/.well-known/ucp manifestinin 'payment methods' hanesine denk gelir).
    key=None → imzasız (iç-dogfood)."""
    t = time.time() if valid_from is None else valid_from
    obj: dict[str, Any] = {
        "intent_id": intent_id,
        "buyer": buyer.lower(),
    ...
    return f"UCP-Checkout {raw}"


def verify_ucp_checkout(header: str, resource: str, *, now: float | None = None,
                        resolve_key: KeyResolver | None = None,
                        require_signature: bool = False,
                        expected_merchant: str | None = None) -> ChargeIntent:
    """UCP başlığı → ChargeIntent (kaynak/tutar + merchant-bağı + imza)."""
    u = UcpCheckout.parse(header)
    u.verify(resolve_key=resolve_key, require_signature=require_signature,
             expected_merchant=expected_merchant)
    return u.authorize(resource, now=now)
...
def install_adapters(register_scheme: Any, *, price_minor: int,
                     ...
                     require_acp_signature: bool = False,
                     require_ucp_signature: bool = False,
                     ucp_merchant: str | None = None) -> None:
    """Üç protokolü middleware şema-Registry'sine takar.
    ...
    on_intent(intent) — protokol-niyeti olayı; SESTER'da
    ...
    require_ucp_signature — UCP satıcı-mührünü zorunlu tutar (üretim: True);
    ucp_merchant — verilirse zarf-merchant'ı buna bağlanır (manifest-bağı).
    ...
    def _ucp(header: str, resource: str) -> dict[str, Any]:
        try:
            u = UcpCheckout.parse(header)
            u.verify(resolve_key=key_resolver,
                     require_signature=require_ucp_signature,
                     expected_merchant=ucp_merchant)
            intent = u.authorize(resource)
        except AdapterError as e:
            raise PaymentErr(f"UCP checkout reddedildi: {e}") from e
        if intent.amount_minor < price_minor:
            raise PaymentErr("UCP line_item tutar satıcı-fiyatının altında")
        if on_intent:
            on_intent(intent)
        return {"agent": intent.agent, "nonce": intent.nonce,
                "amount_minor": intent.amount_minor, "scheme": "ucp",
                "intent": intent}
