"""PUGIO middleware — x402 el sıkışması + şema-kapısı + sayaç + kota (saf ASGI).
...
Akış (özet):
  ...
  şemalar: "exact-pugio" (EIP-191, yerel) | "pugio0" (HMAC geri-uyum) |
...
class PugioMeter:
    ...
    VERSION = "pugio0"          # HMAC şeması (geri-uyum)
    EVM_SCHEME = "exact-pugio"  # EIP-191 imzalı ajan-kimliği
    ...
    def __init__(
        ...
        *,
        ...
        pay_to: str = "pugio:demo-seller",
        facilitator: Any | None = None,   # pugio.facilitator.Facilitator (exact için)
        ...
        self.register_scheme("Pugio-EVM", self._parse_evm)        # "Pugio-EVM <b64>"
    ...
    def _parse_evm(self, header: str, resource: str) -> dict[str, Any]:
        from .schemes import verify_exact_pugio

        info = verify_exact_pugio(header, resource)
    ...
    def _parse_exact(self, header: str, resource: str) -> dict[str, Any]:
        ...
        from .schemes import ExactPugioV2, PaymentError

        try:
            env = ExactPugioV2.parse_payment_header(
                base64.urlsafe_b64encode(json.dumps(envelope).encode()).decode().rstrip("=")
            )
            addr = ExactPugioV2.verify_local(env)
            ...
            "accepts": [
                {
                    ...
                    "extra": {
                        ...
                        "agentHeader": "X-Pugio-Agent (opsiyonel etiket)",
                    },
                },
                {
                    "scheme": self.VERSION,
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        spent_minor = int(round(self.ledger.spent_today(agent) * MINOR))
        ...
        async def send_with_receipt(message):
            if (message["type"] == "http.response.start"
                ...
                headers.append((b"x-pugio-receipt", rec["hash"][:16].encode()))
                headers.append((b"x-pugio-amount", f"{amount_real:.6f}".encode()))
                headers.append((b"x-pugio-seq", str(rec["seq"]).encode()))
