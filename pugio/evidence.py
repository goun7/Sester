"""PUGIO evidence — 81-kanıt-köprüsü v0: dışa-doğrulanabilir kanıt-bundle'ı.

63↔81 kontratı (IS_PLANI §7 S4): PUGIO olayları, agent-filtreli kanıt-segmenti
...
def produce_bundle(ledger: Any, agent_id: str | None = None) -> dict[str, Any]:
    ...
    for ev in events:
        ...
        prev = p
    return {
        "pugio_bundle_version": BUNDLE_VERSION,
        "generated": time.time(),
        "agent": agent_id,
        "head": prev,
...
def verify_bundle(bundle: dict[str, Any]) -> tuple[bool, str]:
    """Alıcı-tarafı: pugio kütüphanesi ve secret OLMADAN doğrula.
    Dönüş: (ok, mesaj)."""
    try:
