"""PUGIO pg_ledger — Postgres backend (v0.3): SQLite-Ledger ile birebir arayüz.

Aynı hash-chain çekirdeği (pugio.ledger.canonical_line + seal) — zincir,
...
Farklar (bilinçli):
  ...
  · bağlantı: PUGIO_PG_DSN env ya da dsn argümanı (lazy — ilk sorguda bağlanır)
Sürücü: psycopg (v3) — `pip install "pugio-meter[pg]"`. psycopg2 fallback.
"""
...
from .ledger import GENESIS, canonical_line, seal
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "pip install 'pugio-meter[pg]'") from e
...
class PgLedger:
    ...
    def __init__(self, db_path: str | os.PathLike[str] | None = None, *,
        ...
        self.dsn = dsn or os.environ.get("PUGIO_PG_DSN")
        if not self.dsn:
            raise RuntimeError("PUGIO_PG_DSN yok: dsn argümanı ya da env şart")
        ...
        with self._lock:  # prev_hash oku→yaz atomik (SQLite-Ledger disiplini)
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    " payload, prev_hash, hash) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"
                    " RETURNING seq",
    ...
    def export_events(self, agent_id: str | None = None) -> list[dict[str, Any]]:
        ...
        return [{"seq": s, "ts": t, "event_type": et, "agent_id": a,
                 ...
                 "hash": ha} for (s, t, et, a, h, am, p, ph, ha) in rows]
