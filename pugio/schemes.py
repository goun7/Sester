"""PUGIO schemes — x402 ödeme-şemaları.

exact-pugio (v0.1, canlı):
  X-PAYMENT: base64url(JSON{scheme:"exact-pugio", agent, nonce, amount, signature})
...
def sign_exact_pugio(secret_key: str, agent: str, nonce: str, amount: str,
                     resource: str) -> str:
    ...
    return f"Pugio-EVM {b64}"


def verify_exact_pugio(header: str, resource: str,
    ...
    if not token.startswith("Pugio-EVM "):
        raise PaymentError("şema-önekli değil")
    raw = token[len("Pugio-EVM "):]
    try:
        padded = raw + "=" * (-len(raw) % 4)
        payload = json.loads(base64.urlsafe_b64decode(padded))
    except (binascii.Error, ValueError) as e:
        raise PaymentError("base64/JSON bozuk") from e
    if payload.get("scheme") != "exact-pugio":
        raise PaymentError("scheme exact-pugio değil")
...
class ExactPugioV2:
    ...
    def client_header(*, from_addr: str, to_addr: str, amount_usd: float,
        ...
        value = ExactPugioV2.encode_amount(amount_usd)
        msgdata = {
