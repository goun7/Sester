"""PUGIO bridges — birleşme-değil, köprü kararı (BIRLESTIRME_DEGERLENDIRMESI.md §3).

K1 · Tamga çıpası:  PUGIO kanıt-segmentinin chain-head + merkle-kökünü
      ...
      external_anchor; PUGIO secret'ı gerekmez, yalnız kamuya proof-alanı).
K2 · Veridict talebi: PUGIO politika/sayaç iddialarını Veridict'in
      makine-doğrulanabilir claim-formatına döker — her claim
      `claim_id = sha256(task_id|summary|verifiability)[:16]` kuralına uyar,
      verifiability=reproducible; kanıt, PUGIO kanıt-bundle'ının head+merkle'ı.
...
pugio'suz doğrulayabilsin (scripts/dogrula.py disiplini).
"""
...
def tamga_anchor(bundle: dict[str, Any], *, agent_label: str | None = None) -> dict[str, Any]:
    ...
    Zarf: tip=external_anchor, kaynak=pugio, head + merkle_root + event_count;
    `anchor_id = sha256(head|merkle_root|event_count)[:32]` — deterministik,
    tekrar-üretilebilir; alıcı yalnız sha256 ile bağlar.
    """
...
def verify_tamga_anchor(anchor: dict[str, Any]) -> tuple[bool, str]:
    ...
    try:
        if anchor.get("type") != "external_anchor" or anchor.get("source") != "pugio":
...
def veridict_claims(bundle: dict[str, Any], *, task_id: str = "pugio-metering") -> dict[str, Any]:
    """PUGIO kanıtını Veridict makine-claim'lerine dök.
