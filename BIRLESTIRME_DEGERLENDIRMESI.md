# 63 · PUGIO — Veridict + Tamga Protocol Birleşme Değerlendirmesi (2026-09-12)
...
| | **Veridict** | **Tamga Protocol** | **PUGIO (63)** |
|---|---|---|---|
...
2. **Üç farklı alıcı, üç farklı keşif-yüzeyi.** Veridict → denetleme/README-tr
   ...
   ekosistemiyle konuşuyor); PUGIO → API-satıcısı. Üç repo = üç yıldız-huni;
...
|---|---|---|---|
| **K0 · Ortak olay-zarfı** | hepsi | canonical satır-formatı + sha256 proof-zinciri + Merkle-kök şartnamesi | ✅ **v0.2'de yazıldı: `docs/K0_SHARED_ENVELOPE_SPEC.md`** (draft v0.1) — PUGIO `evidence.py` referans-uygulama |
| **K1 · PUGIO → Tamga çıpası** | 63→tamga | PUGIO chain-head + merkle-root'unu Tamga work-receipt-zarfı olarak kaydet (deterministik JSONL satırı) | ✅ **kodlandı: `pugio/bridges.py` + Tamga tarafı alıcı `tamga_pugio_receiver.py` (pür stdlib, fail-loud)** — çapraz-repo testli (test_74–76) |
| **K2 · PUGIO → Veridict talebi** | 63→veridict | PUGIO politika-kararları, Veridict'in makine-doğrulanabilir claim formatına dökülür ("quota enforced, replayable") | ✅ **kodlandı: `pugio/bridges.py`** (test_81: claim_id-kuralı format-testi) |
| **K3 · Veridict watcher → PUGIO** | veridict→63 | Veridict watcher'ı PUGIO karar-akışını izler; deny-kararları W2/W3 doktriniyle denetlenir | ✅ **ilk-adım kodlandı: `pugio/watchfeed.py` üretici (K0 §6 zinciri) + Veridict tarafı alıcı `scripts/pugio_watch_receiver.py`** — çapraz-repo testli (test_77–79); tam-watcher-entegrasyonu lansman-sonrası |
| **K4 · Tamga receipt ↔ PUGIO charge** | tamga↔63 | Aynı ajanın work-receipt'leri ile charge-receipt'leri tek evidence-stream'de (`evidence_ref`, IS_PLANI §6 veri-modeli) | ileride |
...
- **Tamga'yı PUGIO'nun ledger'ına gömme:** Tamga'nın WASI-sandbox'ı farklı problem
  (determinizm + taşınabilirlik); gömmek ikisini de küçültür.
- **Veridict'i "sertifika-kütüphanesi" olarak PUGIO'ya bağımlı kılmak:**
...
Kod-karşılığı: `pugio/bridges.py` (K1+K2), `pugio/watchfeed.py` (K3 üretici),
...
> yayınları ve PUGIO vitrini kendi hızlarında sürer — köprü yalnız üç tarafın da
